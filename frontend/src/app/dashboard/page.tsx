...
import { getHistory, type ProjectOut, apiFetch } from "@/lib/api";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    getHistory()
      .then((data) => {
        console.log("[Dashboard] History loaded:", data.length, "projects");
        setProjects(data);
      })
      .catch((e) => {
  ...
  const handleDeleteOne = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        message.error(`Не удалось удалить проект: ${errorData.detail || res.statusText}`);
      }
    } catch (err) {
  ...
  const handleClearAll = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json().catch(() => ({}));
        message.error(`Не удалось очистить историю: ${errorData.detail || res.statusText}`);
      }
    } catch (err) {
                ...
                return (
                  ...
                  >
                    ...
                    <Popconfirm
                      ...
                      onConfirm={(e) =>
                        handleDeleteOne(project.id, e as React.MouseEvent)
                      }
